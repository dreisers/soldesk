...
<%
  ...
  for(int idx=0; idx<num.length; idx++){
   ...
   out.println("<hr>");
   //type=checkbox값이 true면 on, 아니면 null
	out.println("약관동의 : " + request.getParameter("agree"));
	out.println("<br>");
	out.println("성별 : " + request.getParameter("gender"));
	out.println("<br>");
	out.println("통신사 : " + request.getParameter("telecom"));
	out.println("<br>");
	out.println("페이지 : " + request.getParameter("page"));
    /*
		실제 전송된 파일을 가져오려면 다른방식으로 request객체에 접근해야 한다.
		request.getParameter("attach")은 파일을 담고 있는것이 아니라
		파일명을 담고 있을뿐이다.
    */
	out.println("<hr>");
